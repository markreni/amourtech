...
const Social = () => {
  ...
  const direction = isMatch ? 'row' : 'column'
  ...
  const spacing = isSmallest ? 1 : 4
  const size = isSmallest ? 'medium' : 'large'
  const sizeIcons = isSmallest ? 0.7 : 1

  return(
    <Stack direction={direction} alignItems="left" spacing={spacing} sx={{ ...socialMediaStyling, ...socialMediaStylingRow }}>
      ...
      <Stack direction="row" spacing={3} alignItems='center'>
        <Avatar><a href='https://www.soundcloud.com/amourtech' target="_blank" rel="noopener noreferrer"><Icon component='img' src={SoundCloudIcon} color="secondary" sx={{ height: 18 * sizeIcons, width: 34 * sizeIcons, paddingLeft: 0.3, paddingTop: 0.5 }} /></a></Avatar>
        {!isMatch &&
          <Typography variant='caption'>Soundcloud</Typography>
        }
      </Stack>
      <Stack direction="row" spacing={3} alignItems='center'>
        <Avatar><a href='https://www.mixcloud.com/amourtech' target="_blank" rel="noopener noreferrer"><Icon component='img' src={MixCloudIcon} color="secondary" sx={{ height: 35 * sizeIcons, width: 35 * sizeIcons, paddingLeft: 0.3, paddingTop: 1.2 }} /></a></Avatar>
        {!isMatch &&
